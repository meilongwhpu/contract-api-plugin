...
//	private Map<String, Object> map;
	...
	/**
	 * 发送文件的方式：1-发送InputStream对象，2-发送文件
	 */
	...
	public HttpConfig json(String json) {
		...
		map.put(HttpClientUtils.ENTITY_JSON, json);
		maps.set(map);
		return this;
	}

	/**
	 * 设置上传文件的方式 1-发送InputStream对象，2-发送文件
	 * @param mode
	 * @return
	 */
	public HttpConfig fileMode(int mode){
		this.fileMode=mode;
		return this;
	}
	...
	public HttpConfig files(String[] filePaths, String inputName, boolean forceRemoveContentTypeChraset) {
		...
		m.put(HttpClientUtils.ENTITY_MULTIPART, filePaths);
		m.put(HttpClientUtils.ENTITY_MULTIPART+".name", inputName);
		m.put(HttpClientUtils.ENTITY_MULTIPART+".rmCharset", forceRemoveContentTypeChraset);
		maps.set(m);
		return this;
	}
