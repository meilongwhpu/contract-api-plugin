package io.nuls.contract.httpclient.common;
...
public class HttpClientUtils {
	 ...
	 */
	public static String checkHasParas(String url, List<NameValuePair> nvps, String encoding,int fileMode) throws UnsupportedEncodingException {
		// 检测url中是否存在参数
		if (url.contains("?") && url.indexOf("?") < url.indexOf("=")) {
			Map<String, Object> map = buildParas(url.substring(url.indexOf("?") + 1));
			InputStream inputStream=null;
			try{
				map2HttpEntity(nvps, map, encoding,fileMode,inputStream);
			}finally {
				if(inputStream != null){
					try {
						inputStream.close();
	 ...
	 * @throws UnsupportedEncodingException  不支持的编码异常
	 */
	public static HttpEntity map2HttpEntity(List<NameValuePair> nvps, Map<String, Object> map, String encoding,int fileMode,InputStream inputStream) throws UnsupportedEncodingException {
					...
					if(ENTITY_STRING.equals(entry.getKey())){//string
						...
						if(files.length==1){
							try{
								...
								//发送InputStream对象
								if(fileMode==1){
									builder.addBinaryBody(String.valueOf(map.get(ENTITY_MULTIPART+".name")),inputStream);
								}else{
									builder.addBinaryBody(String.valueOf(map.get(ENTITY_MULTIPART+".name")) ,files[0]);
								}
...
//							builder.addPart(String.valueOf(map.get(ENTITY_MULTIPART+".name")) + count++, fileBody);
								try{
									...
									//发送InputStream对象
									if(fileMode==1){
										builder.addBinaryBody(String.valueOf(map.get(ENTITY_MULTIPART+".name")) + count++,inputStream);
									}else{
