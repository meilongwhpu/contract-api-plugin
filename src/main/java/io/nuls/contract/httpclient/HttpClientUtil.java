package io.nuls.contract.httpclient;

import io.nuls.contract.httpclient.common.HttpClientUtils;
import io.nuls.contract.httpclient.common.HttpConfig;
import io.nuls.contract.httpclient.common.HttpMethods;
import io.nuls.contract.httpclient.common.HttpResult;
...
public class HttpClientUtil{
	...
	static{
		try {
			client4HTTP = HttpClientCreater.custom().build();
			client4HTTPS = HttpClientCreater.custom().ssl().build();
		} catch (HttpProcessException e) {
			HttpClientUtils.errorException("创建https协议的HttpClient对象出错：{}", e);
		}
	}
	...
	public static String get(HttpClient client, String url, Header[] headers, HttpContext context, String encoding) throws HttpProcessException {
		return get(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).context(context).encoding(encoding));
	}
	/**
	 * 以Get方式，请求资源或服务
	 * 
	...
	public static String post(HttpClient client, String url, Header[] headers, Map<String,Object>parasMap, HttpContext context, String encoding) throws HttpProcessException {
		return post(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).map(parasMap).context(context).encoding(encoding));
	}
	/**
	 * 以Post方式，请求资源或服务
	 * 
	...
	public static String put(HttpClient client, String url, Map<String,Object>parasMap,Header[] headers, HttpContext context,String encoding) throws HttpProcessException {
		return put(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).map(parasMap).context(context).encoding(encoding));
	}
	/**
	 * 以Put方式，请求资源或服务
	 * 
	...
	public static String delete(HttpClient client, String url, Header[] headers, HttpContext context,String encoding) throws HttpProcessException {
		return delete(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).context(context).encoding(encoding));
	}
	/**
	 * 以Delete方式，请求资源或服务
	 * 
	...
	public static String patch(HttpClient client, String url, Map<String,Object>parasMap, Header[] headers, HttpContext context,String encoding) throws HttpProcessException {
		return patch(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).map(parasMap).context(context).encoding(encoding));
	}
	/**
	 * 以Patch方式，请求资源或服务
	 * 
	...
	public static String head(HttpClient client, String url, Header[] headers, HttpContext context,String encoding) throws HttpProcessException {
		return head(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).context(context).encoding(encoding));
	}
	/**
	 * 以Head方式，请求资源或服务
	 * 
	...
	public static String options(HttpClient client, String url, Header[] headers, HttpContext context,String encoding) throws HttpProcessException {
		return options(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).context(context).encoding(encoding));
	}
	/**
	 * 以Options方式，请求资源或服务
	 * 
	...
	public static String trace(HttpClient client, String url, Header[] headers, HttpContext context, String encoding) throws HttpProcessException {
		return trace(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).context(context).encoding(encoding));
	}
	/**
	 * 以Trace方式，请求资源或服务
	 * 
	...
	public static OutputStream down(HttpClient client, String url, Header[] headers, HttpContext context, OutputStream out) throws HttpProcessException {
		return down(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).context(context).out(out));
	}
	...
	public static String upload(HttpClient client, String url, Header[] headers, HttpContext context) throws HttpProcessException {
		return upload(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).context(context));
	}
	...
	public static int status(HttpClient client, String url, Header[] headers, HttpContext context, HttpMethods method) throws HttpProcessException {
		return status(HttpConfig.initHttpConfig().client(client).url(url).headers(headers).context(context).method(method));
	}
		...
		try {
			...
			if(HttpEntityEnclosingRequestBase.class.isAssignableFrom(request.getClass())){
				...
				if(request.getClass()==HttpGet.class) {
					...
					config.url(HttpClientUtils.checkHasParas(config.url(), nvps, config.inenc(),config.getFileMode()));
				}
				
				//装填参数
				HttpEntity entity = HttpClientUtils.map2HttpEntity(nvps, config.map(), config.inenc(),config.getFileMode(),inputStream);
				
				//设置参数到请求对象中
				((HttpEntityEnclosingRequestBase)request).setEntity(entity);
				
				HttpClientUtils.info("请求地址："+config.url());
				if(nvps.size()>0){
					HttpClientUtils.info("请求参数："+nvps.toString());
				}
				if(config.json()!=null){
					HttpClientUtils.info("请求参数："+config.json());
				}
			}else{
				int idx = config.url().indexOf("?");
				HttpClientUtils.info("请求地址："+config.url().substring(0, (idx>0 ? idx : config.url().length())));
				if(idx>0){
					HttpClientUtils.info("请求参数："+config.url().substring(idx+1));
				}
			}
			//执行请求操作，并拿到结果（同步阻塞）
